...
<table class='map_designer_section right_side'>";
	$labels_ctr = 0;
	foreach ($sl_mdo['labels'] as $key => $value) {
		if ($labels_ctr % 3 == 0) {
			$the_row_id = (!empty($value["row_id"]))? " id = '$value[row_id]' " : "";
			print "<tr {$the_row_id}>";
		}	
		...
		print "<td>".$value['input_template']."<br><span style='font-size:80%'>".__($value['label'], SL_TEXT_DOMAIN)."{$the_more_info_label}</span>";
