...
/*----------------------------*/
function sl_do_geocoding($address, $sl_id="", $lat="", $lng="") {
	...
	$sl_id = ($sl_id==="")? $wpdb->insert_id : $sl_id ;
	$GLOBALS['sdg_delay'] = (empty($GLOBALS['sdg_delay']))? 0 : $GLOBALS['sdg_delay'];
	//$sl_ccTLD = (empty($sl_vars['map_region']))? "us" : $sl_vars['map_region'] ;
	if (empty($GLOBALS['sdg_loaded']) || $GLOBALS['sdg_loaded']===0){
		print "<div id='sdg_status'></div><script src='https://maps.googleapis.com/maps/api/js?v=3{$sens}{$lang_loc}{$region_loc}{$key}' type='text/javascript'></script>
		<!--script>
		if (!(typeof google === 'object' && typeof google.maps === 'object')) {
			jQuery.getScript('https://maps.googleapis.com/maps/api/js?v=3{$sens}{$lang_loc}{$region_loc}{$key}');
			
		}</script-->";
print "<script>console.log('loading: Google Maps');var sl_geocoder;
sl_geocoder = new google.maps.Geocoder(); 
var sl_ccTLD = \"$sl_vars[map_region]\";
function sdg_js(address, sl_id){
	sl_geocoder.geocode({'address':address, 'region': sl_ccTLD}, function(results,status) {
		center = results[0].geometry.location;  console.log('lat:'+ center.lat() + ' | lng:' + center.lng());
		jQuery.get('".$_SERVER['REQUEST_URI']."&sdg=1&lat=' + center.lat() + '&lng=' + center.lng() + '&i=$sl_id', function(data){
			//document.getElementById('sdg_status').innerHTML += '<br>' + data;  console.log('inside status msg:' + document.getElementById('sdg_status').innerHTML);
			document.getElementById('sl_tr-'+sl_id).bgColor='white';
		});
	});
}
</script>";
	}
	print "<script>setTimeout(function(){sdg_js(\"$address\", '$sl_id');},$GLOBALS[sdg_delay]); </script>\n";
	$GLOBALS['sdg_loaded'] = 1;
	$GLOBALS['sdg_delay'] += 700;
    }
}
if (!empty($_GET['sdg'])) {
	...
	sl_define_db_tables();
	if (!empty($_GET['i'])) {
		sl_do_geocoding("", $_GET['i'], $_GET['lat'], $_GET['lng']);		
	} else {
		sl_do_geocoding("", "", $_GET['lat'], $_GET['lng']);
	}
	//exit();
}
function sl_do_geocoding_real($address,$sl_id="") {
