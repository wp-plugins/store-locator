 ...
 function sl_do_geocoding($address, $sl_id="") {
   if (empty($_POST['no_geocode']) || $_POST['no_geocode']!=1){
    ...
    if (strcmp($status, "OK") == 0) {
		...
		/*print "<br><b>Second Attempt ...</b> <span id='sl_second_pass_status'></span><br><br>";
		$sens=(!empty($sl_vars['sensor']) && ($sl_vars['sensor'] === "true" || $sl_vars['sensor'] === "false" ))? "&amp;sensor=".$sl_vars['sensor'] : "&amp;sensor=false" ;
		$lang_loc=(!empty($sl_vars['map_language']))? "&amp;language=".$sl_vars['map_language'] : "" ; 
		$region_loc=(!empty($sl_vars['map_region']))? "&amp;region=".$sl_vars['map_region'] : "" ;
		$key=(!empty($sl_vars['api_key']))? "&amp;key=".$sl_vars['api_key'] : "" ;
...
//		print "<script src='".SL_JS_BASE."/store-locator.js?v=".SL_VERSION."'>";
		print "<script type='text/javascript'>
			//window.location.hash = ''; //prevent scrolling
			//jQuery('a[name=a".$_GET['edit']."]').remove();
			sl_geocoder = new google.maps.Geocoder();
			sl_geocoder.geocode( {'address': \"".trim($address)."\"}, function(results, status) {
				//alert(status);
				if (status != google.maps.GeocoderStatus.OK) {
					center = results[0].geometry.location;
					jQuery.get('".SL_INCLUDES_BASE."/sl-second-pass-geo.php?lat=' + center.lat() + '&lon=' + center.lng(), function() {
						document.getElementById('sl_second_pass_status').innerHTML = \"<font color='DarkGreen'>Success!</font>\";
						setTimeout(function(){location.replace('".str_replace("&edit=$_GET[edit]", "", $_SERVER['REQUEST_URI'])."');}, 3000);
					});
					
				} else {
					document.getElementById('sl_second_pass_status').innerHTML = \"<font color='red'>Failed again. Check <a href='https://".$sl_vars['google_map_domain']."?q=".urlencode(trim($address))."' target='_blank'>Google search</a> to make sure address is valid.</font>\";
					setTimeout(function(){location.replace('".str_replace("&edit=$_GET[edit]", "", $_SERVER['REQUEST_URI'])."');}, 3000);
				}
			}); 
			if (location.hash) {
