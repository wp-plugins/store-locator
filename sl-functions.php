...
if (!isset($sl_vars['scripts_load_home']) || strlen(trim($sl_vars['scripts_load_home'])) == 0) {	$sl_vars['scripts_load_home']="1";	}
$sl_scripts_load_home=$sl_vars['scripts_load_home'];

if (!isset($sl_vars['scripts_load_archives_404']) || strlen(trim($sl_vars['scripts_load_archives_404'])) == 0) {	$sl_vars['scripts_load_archives_404']="1";	}
$sl_scripts_load_archives_404=$sl_vars['scripts_load_archives_404'];

if (empty($sl_vars['hours_label']) || strlen(trim($sl_vars['hours_label'])) == 0) {	$sl_vars['hours_label']="Hours";	}
$sl_hours_label=$sl_vars['hours_label'];

if (empty($sl_vars['phone_label']) || strlen(trim($sl_vars['phone_label'])) == 0) { $sl_vars['phone_label']="Phone";	}
$sl_phone_label=$sl_vars['phone_label'];

if (empty($sl_vars['fax_label']) || strlen(trim($sl_vars['fax_label'])) == 0) {	$sl_vars['fax_label']="Fax";	}
$sl_fax_label=$sl_vars['fax_label'];

if (empty($sl_vars['email_label']) || strlen(trim($sl_vars['email_label'])) == 0) {	$sl_vars['email_label']="Email";	}
$sl_email_label=$sl_vars['email_label'];
...
function sl_head_scripts() {
	...
	if ( (!empty($sl_vars['scripts_load']) && $sl_vars['scripts_load'] == 'all') || $on_sl_page || is_search() || ((is_archive() || is_404()) && $sl_code_is_used_in_posts && (!isset($sl_vars['scripts_load_archives_404']) || $sl_vars['scripts_load_archives_404']==1)) || (is_front_page() && (!isset($sl_vars['scripts_load_home']) || $sl_vars['scripts_load_home']==1)) || is_single($post_ids_array) || (is_singular() && !is_singular(array('page', 'attachment', 'post'))) || function_exists('show_sl_scripts') || is_page_template()) {
...
function sl_dyn_js($post_content=""){
	...
	$hl=(trim($sl_vars['hours_label'])!="")? parseToXML($sl_vars['hours_label']) : "Hours";
	$pl=(trim($sl_vars['phone_label'])!="")? parseToXML($sl_vars['phone_label']) : "Phone";
	$fl=(trim($sl_vars['fax_label'])!="")? parseToXML($sl_vars['fax_label']) : "Fax";
	$el=(trim($sl_vars['email_label'])!="")? parseToXML($sl_vars['email_label']) : "Email";	
...
var sl_hours_label='$hl';
var sl_phone_label='$pl';
var sl_fax_label='$fl';
var sl_email_label='$el';
...
function sl_remote_data($val_arr, $decode_mode = 'json') {
	...
	if (extension_loaded("curl") && function_exists("curl_init")) {
    			ob_start();
    			$ch = curl_init();
    			if (!empty($useragent)){ curl_setopt($ch, CURLOPT_USERAGENT, $useragent); }
			...
			if (!empty($useragent)){ $http_request .= "User-Agent: $useragent\r\n"; }
			$http_request .= "\r\n";
