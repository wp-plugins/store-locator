...
function sl_head_scripts() {
	...
	$on_sl_page=$wpdb->get_results("SELECT post_name, post_content FROM ".SL_DB_PREFIX."posts WHERE LOWER(post_content) LIKE '%[store-locator%' AND post_status IN ('publish', 'draft') AND (post_name='$pagename' OR ID='".esc_sql($_GET['p'])."' OR ID='".esc_sql($_GET['page_id'])."')", ARRAY_A);		
...
function sl_location_form($mode="add", $pre_html="", $post_html=""){
	...
	<tr>
		...
		<b>".__("The General Address Format", SL_TEXT_DOMAIN).": </b>onclick='jQuery(\"#sl_info_news\").slideToggle();' style='cursor:hand; cursor:pointer;'(<a href=\"#\" onclick=\"show('format'); return false;\">".__("show/hide", SL_TEXT_DOMAIN)."</a>)
