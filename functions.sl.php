...
global $search_label, $zoom_level;
...
function ajax_map($content) {
	...
	global $sl_dir, $sl_base, $sl_path, $text_domain;
...
<div id='sl_div'>
  <form onsubmit='searchLocations(); return false;'>
    <table width=1><tr>
	<td valign=top><nobr>$search_label</nobr></td><td colspan='2'><input type='text' id='addressInput' size=35/></td>
	</tr><tr>
	 <td valign=top>".__("Radius", $text_domain)."</td><td width='33%' valign='top'><select id='radiusSelect'>
	$r_options
    </select>
	</td>
...
function sl_add_options_page() {
	...
	if (trim($api)!=""){
		...
		add_submenu_page($sl_dir.'/news-upgrades.php', __("View Locations", $text_domain), __("View Locations", $text_domain), 9, $sl_dir.'/view-locations.php');
	...
	add_submenu_page($sl_dir.'/news-upgrades.php', __("Localization", $text_domain)."&nbsp;&amp;&nbsp;".__("Enter Google API Key", $text_domain),  __("Localization", $text_domain)."&nbsp;&amp;&nbsp;".__("Enter Google API Key", $text_domain), 9, $sl_dir.'/api-key.php');
	if (trim($api)!=""){
