...
print "<br>";
$style_var = "";
if (!empty($_POST['sl_thanks'])) {sl_data('sl_thanks', 'add', $_POST['sl_thanks']); unset($_POST);}
$sl_thanks=sl_data('sl_thanks');
print <<<EOQ
<ul class="tablist">\n
EOQ;
$ctr=0; $tsn_links_js="<script>\nvar tsn_link_arr = [];"; $tsn_links_output="";
//$tm_st = ((time() - strtotime(sl_data("sl_start")))/60/60/24>=30);
...
//$thnks_ct=sl_data('sl_thanks_count');
/*if (empty($thnks_ct) || $thnks_ct<=20){
	if (!empty($thnks_ct)){ sl_data('sl_thanks_count', 'update', ($thnks_ct+1)); } else { sl_data('sl_thanks_count', 'add', 1); }
} elseif ($thnks_ct==21) {
	sl_data('sl_thanks_count', 'update', ($thnks_ct+1));
...
//if (preg_match("@addon-settings@", $_GET['page'])){
	...
	if ($sl_thanks!="false" && $sl_thanks!="true") {
		//&& $tm_st
		print "<input rel='sl_pop' type='button' class='button-primary' onclick='return false;' id='thanks_button' href='".SL_INCLUDES_BASE."/thank-you.php?ajax=true' style='margin-right:10px; font-weight:bold; margin:5px; background:green; float:right;' value='   Thank You   '/>";
	}
	print "</div>";
//}
...
<div style='background-color:lightYellow; /*border-top:solid gold 1px; border-bottom:solid gold 1px; */padding:7px'><strong style='font-size:1.0em'>B.</strong>&nbsp;<b>Boost capabilities:</b> <a href='http://www.viadat.com/products-page/' target='_blank'>LotsOfLocales&trade; Addons Platform</a>. 11+ free Flagship addons and counting, featuring your most requested upgrades & customizations (<a href='#las-info' rel='sl_pop'>More Information</a>)</div>
...
<li>Learn more about Flagship Addons that come with the purchase of the LotsOfLocales&trade; Addons Platform from the 'ReadMe Instructions' in the pull-out Dashboard</li>
</ol>
</div>";
}
...
<div id="slideout">
	...
	<div id="slidecontent">
	   <div id='slidecontainer'>
		<div style='width:50%; float:left'><?php sl_module("thanks", "Thank You for Using LotsOfLocales&trade;", "240px");  ?></div>
